classdef DataReader < handle
    ...
    properties
        ...
        %   )
        %   Pixel positions of spots, objects, and stars for each frame
    ...
    methods
        ...
        function initFilenames(this, filename)
            ...
            this.filenames = struct(...
                ...
                'isObjFig', fullfile(this.folders.results, [name, '-isobj.fig']), ...
                'fluxHistFig', fullfile(this.folders.results, [name, '-flux-hist.fig']), ...
                'fluxBoxFig', fullfile(this.folders.results, [name, '-flux-box.fig']), ...
                'magHistFig', fullfile(this.folders.results, [name, '-mag-hist.fig']), ...
                'magBoxFig', fullfile(this.folders.results, [name, '-mag-box.fig']));
        end
        
        % Data
        function initData(this)
            load(this.filenames.data, 'SampleData');
            this.data = SampleData;
        end
        
        % Number of frames
        function initN(this)
            times = this.data.AlgorithmInputData.Time;
            this.N = numel(times);
        end
        ...
        function C = getCentroidXY(this, spotType)
            switch spotType
                case 'object'
                    C = this.data.Objects.Centroid_XY; % [object, 2, time]
                    C = permute(C, [3, 1, 2]); % [time, object, 2]
                case 'star'
                    C = this.data.Stars.Centroid_XY; % [2, star, time]
                    C = permute(C, [3, 2, 1]); % [time, star, 2]
            end
        end
    ...
    methods
        function plotIsObj(this)
            ...
            savefig(this.filenames.isObjFig);
        end
    end
    ...
    methods
        function histFlux(this)
            ...
            savefig(this.filenames.fluxHistFig);
        end
        ...
        function boxFlux(this)
            ...
            savefig(this.filenames.fluxBoxFig);
        end
    ...
    methods
        function histMag(this)
            ...
            savefig(this.filenames.magHistFig);
        end
        ...
        function boxMag(this)
            ...
            savefig(this.filenames.magBoxFig);
        end
    ...
    methods
        ...
        function spotPixelOverDilatedVideo(this, labelFlag)
            if nargin < 2
                labelFlag = false;
            end
...
%                 imshow(frame);
    ...
    methods(Static)
        function main()
            ...
            filename = './assets/data/SampleDataRun5.mat';
            ...
            % pixel position
            dr.makeVideo();
            dr.spotPixelOverDilatedVideo(false);
            dr.spotPixelOverDilatedVideo(true);
        end
        ...
        function amos19()
            ...
            % filename = './assets/data-old/SampleData3.mat'; % simple-fg
            % filename = './assets/data-old/SampleData4.mat'; % complex-fg
            % filename = './assets/data/SampleDataRun2.mat'; % simple-bg
            filename = './assets/data/SampleDataRun1.mat'; % simple-bg
            dr = DataReader(filename);
            ...
            fprintf('Number of spots: %.1f $\\pm$ %.1f\n', mean(ns), std(ns));
            ...
            fprintf('Number of objects: %.1f $\\pm$ %.1f\n', mean(no), std(no));
        end
    end
end
